...
// Now apply it ONLY to the first one (Amount)
// We can find the comment {/* Amount */}
content = content.replace(
  /\{\/\* Amount \*\/\}\n(\s*)<td className="px-6 py-4 whitespace-nowrap">/g,
  `{/* Checkbox */}\n$1<td className="px-6 py-4 w-12"><input type="checkbox" className="rounded border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900" checked={selectedIds.includes(invoice.id)} onChange={(e) => { if (e.target.checked) setSelectedIds([...selectedIds, invoice.id]); else setSelectedIds(selectedIds.filter(id => id !== invoice.id)); }} onClick={(e) => e.stopPropagation()} /></td>\n$1{/* Amount */}\n$1<td className="px-6 py-4 whitespace-nowrap">`
);
