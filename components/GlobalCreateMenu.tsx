...
export default function GlobalCreateMenu() {
  ...
  return (
    <div className="fixed bottom-8 right-8 z-50 print:hidden" ref={menuRef}>
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2 bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded-full font-bold shadow-lg shadow-primary/20 transition-all hover:scale-105"
      >
        <Plus size={18} /> Create
      </button>

      {isOpen && (
        <div className="absolute right-0 bottom-full mb-2 md:mb-2 w-56 bg-white dark:bg-zinc-900 rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-800 py-2 animate-in fade-in slide-in-from-bottom-2 duration-200">
