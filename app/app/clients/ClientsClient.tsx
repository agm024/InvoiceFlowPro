              ...
              <div key={client.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col group">
                <div className="p-5 border-b border-card-border">
                  <div className="flex justify-between items-start mb-2">
                    <Link href={`/app/clients/${client.slug}`} className="font-bold text-lg text-foreground hover:text-zinc-900 dark:text-white transition-colors truncate pr-2">
                      {client.name}
                    </Link>
                  ...
                  <div className="flex justify-end opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity mt-1 mb-2">
                      ...
                      <button 
                        onClick={() => { if (!client.portalToken) return toast.error('Token not generated yet.'); if (!client.email) return toast.error('Client has no email address.'); setSelectedClientForEmail(client); setEmailSubject('Your Client Portal Access'); setEmailMessage('Here is the link to access your dedicated Client Portal. You can view your active projects, estimates, outstanding invoices, and statement of accounts.'); setEmailModalOpen(true); }}
