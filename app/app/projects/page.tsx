  ...
  return (
    <div className="p-8 max-w-6xl mx-auto w-full">
      ...
      <div className="bg-card-bg border border-card-border rounded-xl shadow-sm overflow-hidden">
                ...
                projects.map((project) => {
                  ...
                  return (
                    <tr key={project.id} className="hover:bg-sidebar-bg/50 transition-colors group cursor-pointer">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${isCompleted ? 'bg-emerald-500/10 text-emerald-600' : 'bg-zinc-100 dark:bg-zinc-800/10 text-zinc-900 dark:text-white'}`}>
        ...
        {/* Mobile View: Cards */}
        <div className="block md:hidden divide-y divide-sidebar-border">
          {projects.length === 0 ? (
              ...
              return (
                <div key={project.id} className="p-4 flex flex-col gap-3">
                  ...
                  <div className="flex justify-between items-end mt-1">
                    <div className="text-xs text-zinc-500">{project.milestones.length} Phases</div>
                    <div className="text-right">
                      <div className="font-bold text-foreground tabular-nums">₹ {project.totalValue.toLocaleString()}</div>
                      <div className="text-xs text-zinc-500 tabular-nums mt-0.5">₹ {billedSum.toLocaleString()} Billed</div>
                    </div>
                  </div>
                </div>
              )
