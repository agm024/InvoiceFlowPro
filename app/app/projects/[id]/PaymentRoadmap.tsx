            ...
            <div key={milestone.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/20">
              ...
              <div className="flex items-center gap-3 shrink-0">
                {isPaid ? (
                  ...
                  <Link href={`/app/invoices/${milestone.invoiceId}`} className="text-sm font-bold text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors px-4 py-2 rounded-xl border border-blue-200 dark:border-blue-800 flex items-center gap-2">
                    <FileText size={16} /> View Invoice
                  </Link>
                ) : (
                  <Link href={`/app/invoices/new?milestoneId=${milestone.id}`} className="text-sm font-bold text-zinc-900 dark:text-white bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors px-4 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm flex items-center gap-2">
