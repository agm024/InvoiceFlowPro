...
export default async function NewProjectPage({ searchParams }: { searchParams: Promise<{ clientId?: string }> }) {
  ...
  const initialClientId = resolvedParams.clientId || '';
  ...
  return (
    <div className="p-8 md:p-12 max-w-7xl mx-auto w-full">
      <div className="mb-10 flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-zinc-200 dark:bg-zinc-800 text-zinc-900 dark:text-primary-foreground dark:bg-primary/50 dark:text-zinc-300 text-xs font-bold px-2 py-0.5 rounded-full tracking-wide uppercase">New Contract</span>
          </div>
      ...
      <ProjectForm clients={clients} initialClientId={initialClientId} />
    </div>
  )
}
