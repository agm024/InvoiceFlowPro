                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <div className="relative group/dropdown">
                        <button className="p-1.5 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md transition-colors">
                          <MoreHorizontal size={16} />
                        </button>
                        <div className="absolute right-0 top-full mt-1 w-40 bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-800 opacity-0 invisible group-hover/dropdown:opacity-100 group-hover/dropdown:visible transition-all z-50 overflow-hidden">
                          {invoice.client.email && (
                            <button onClick={() => openEmailModal(invoice)} className="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-left w-full border-b border-zinc-100 dark:border-zinc-800">Email Document</button>
                          )}
                          <Link href={`/app/invoices/${invoice.id}/edit`} className="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-left w-full border-b border-zinc-100 dark:border-zinc-800">Edit</Link>
                          <button onClick={() => handleDelete(invoice.id)} className="block px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 text-left w-full">Delete</button>
                        </div>
                      </div>
                    </div>
                  </td>
