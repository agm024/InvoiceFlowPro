...
import { createPlatformRole, updatePlatformRole, deletePlatformRole } from "./actions"

const AVAILABLE_PERMISSIONS = [
  { id: "MANAGE_PLATFORM_SETTINGS", label: "Manage Platform Settings" },
  { id: "MANAGE_TENANTS", label: "Manage Tenants & Businesses" },
  { id: "MANAGE_GLOBAL_USERS", label: "Manage Global Users" },
  { id: "MANAGE_SUBSCRIPTIONS", label: "Manage Billing & Subscriptions" },
...
export default function RolesClient({ initialRoles }: { initialRoles: any[] }) {
  ...
  const handleSave = async () => {
    ...
    const savePromise = isCreating 
      ? createPlatformRole(formData)
      : updatePlatformRole(editingRole, formData)
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this global role? Admins with this role will lose their custom permissions.")) return
    
    toast.promise(deletePlatformRole(id), {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-zinc-900 dark:text-white flex items-center gap-2">
          <Shield className="w-5 h-5 text-primary" /> Global Platform Roles
        </h2>
      ...
      {(isCreating || editingRole) && (
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
          <h3 className="text-lg font-semibold mb-4">{isCreating ? "Create New Global Role" : "Edit Global Role"}</h3>
          ...
          <div className="space-y-4 max-w-2xl">
            ...
            <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800">
              <label className="block text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-3">Platform Permissions</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {roles.length === 0 ? (
            <div className="col-span-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-12 text-center text-zinc-500 shadow-sm">
              ...
              <p>No global platform roles created yet.</p>
            </div>
          ) : roles.map((role: any) => {
