...
import { signUpAction, setSignUpCookie } from './actions'
...
export default function SignUpPage() {
  ...
  const handleSendOtp = async () => {
    ...
    if (!has8Chars) return toast.error('Password must be at least 8 characters')
          ...
          <div className="space-y-4">
                  ...
                  <div className="flex-1 h-px bg-zinc-200 dark:bg-zinc-800"></div>
                    ...
                    <div className="mt-6 flex flex-col gap-3">
                      <button onClick={async () => {
                          try {
                            // Set a cookie so auth.ts knows it's a signup flow
                            document.cookie = "isSignUp=true; path=/; max-age=300"
                            const searchParams = new URLSearchParams(window.location.search)
                            const planId = searchParams.get('planId')
                            const cycle = searchParams.get('cycle')
                            let callbackUrl = '/onboarding'
                            if (planId) callbackUrl += `?planId=${planId}&cycle=${cycle || 'monthly'}`
                            signIn('google', { callbackUrl });
                          } catch (e) {
                            toast.error('Something went wrong');
                          }
                      }} type="button" className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 px-4 py-3 rounded-xl font-medium hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors flex items-center justify-center gap-3">
                      ...
                      }} type="button" className="w-full bg-[#24292F] text-white px-4 py-3 rounded-xl font-medium hover:bg-[#24292F]/90 transition-colors flex items-center justify-center gap-3">
            ...
            {step === 4 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <Turnstile siteKey={process.env.NODE_ENV === 'development' ? '1x00000000000000000000AA' : (process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY || '1x00000000000000000000AA')} options={{ action: 'signup' }} onSuccess={(token) => setTurnstileToken(token)} onExpire={() => setTurnstileToken(null)} onError={() => setTurnstileToken(null)} />
